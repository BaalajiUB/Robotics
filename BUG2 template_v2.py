...
def laser_callback(laser):
    ...
    if wall == 1 and flg == 0:
        ...
        if x1==x2:
            ...
            slope_angle=0
            if x1>x2:
                slope_angle = math.atan((y1-y2)/(x1-x2))
            elif x1<x2:
                slope_angle = math.atan((y2-y1)/(x2-x1))
...
def odom_callback(odom):
    global line,r1,ent,bot_x,bot_y,curr_orientation,target_rotation
    ...
    if wall==1 and ent==1:
        angle_1 = target_rotation
        if angle_1>0: #compute
            rotate_2()
            return
        straight()
